    ...
    case "$1" in
        ...
        --proxy-host)
            if [ -n "$proxy_host" ]; then
                echo "Error: you have already entered value for --proxy-host"
                exit 1
            else
                proxy_host=$2
                use_proxy=true
        ...
        --proxy-port)
            if [ -n "$proxy_port" ]; then
                echo "Error: you have already entered value for --proxy-port"
                exit 1
            else
                proxy_port=$2
                shift 2
...
if [ $use_proxy = true ]; then
    if [ -z "$proxy_host" -o -z "$proxy_port" ]; then
        echo "Error: you must specify --proxy-host and --proxy-port at the same time"
        exit 1
    fi
    echo -e "\e[0;32mset proxy http://$proxy_host:$proxy_port/ ...\e[0m"
...
Optional arguments:
   ...
   --proxy-host ip/hostname                Set the proxy host to be used (must be set together with \`--proxy-port\`) (default: none)
   --proxy-port port                       Set the proxy port to be used (must be set together with \`--proxy-host\`) (default: none)
